...
from prior.utils.vq_adapter import load_vq_experiment, core_model, get_vq_info, indices_to_latent_sum
...
def evaluate(
    ...
    for batch in loader:
        ...
        loss = diffusion.training_loss(model, z0, t=t, cond=cond, mask=token_mask)
...
def main():
    ...
    ap.add_argument("--config", type=str, default=str(Path(__file__).resolve().parent / "configs" / "diffusion_prior.yaml"))
    ...
    if num_q != vq_info.num_quantizers and rank == 0:
        print(f"[warn] config num_quantizers={num_q} != vq num_quantizers={vq_info.num_quantizers}, using vq")
    ...
    if is_dist():
        ...
        sampler = DistributedSampler(train_ds, num_replicas=get_world_size(), rank=rank, shuffle=True, drop_last=False)
    ...
    train_loader = DataLoader(
        ...
        drop_last=False,
        collate_fn=lambda b: collate_pad(b, pad_id=pad_id, geo_dim=geo_dim, multiple_of=num_q),
    ...
    while step < max_updates:
        ...
        if grad_clip > 0:
            ...
            torch.nn.utils.clip_grad_norm_((model.module if isinstance(model, DDP) else model).parameters(), grad_clip)
        ...
        if rank == 0 and val_loader is not None and (eval_interval > 0) and (step > 0) and (step % eval_interval == 0):
            to_eval = model.module if isinstance(model, DDP) else model
            if ema is not None:
                backup = {k: v.detach().clone() for k, v in to_eval.state_dict().items()}
                ema.copy_to(to_eval)
                val_loss = evaluate(to_eval, diffusion, vq_codebook, num_q, pad_id, val_loader, device, use_geo=use_geo)
