...
Inputs (from cache .npz):
  ...
  - latents:       [N,D] optional, not used for plots here
...
Outputs (saved under <cache_dir>/analysis/):
  ...
  - tsne_len_highlight_<min>_<max>.png   (if highlight range is given)
  - tsne_helix_frac.png
  - tsne_sheet_frac.png
  - tsne_loop_frac.png
  - tsne_ss_argmax.png
...
The "island" clusters are defined as all clusters except the largest one
(according to KMeans cluster sizes).

Example:

python scripts/analyze_tsne_cache.py \
  --cache latent_analysis/class1/tsne_cache_class1_len_between_1_80.npz \
  --n_clusters 6 \
  --top_k_folds 5 \
  --highlight_len_min 1 \
  --highlight_len_max 30
"""
...
def plot_tsne_plain(lat2d, out_png, title="t-SNE plain"):
    """Basic shape-only t-SNE scatter with a single color."""
...
def plot_tsne_continuous(lat2d, values, out_png, title, cbar_label):
    """t-SNE colored by a continuous scalar (e.g. length, helix_frac)."""
...
def plot_tsne_highlight_length(lat2d, lengths, len_min, len_max, out_png):
    """
    t-SNE where points in [len_min, len_max] are highlighted with a bright color,
    and all other points are shown in light gray.

    If no points fall into the range, a gray-only plot is still generated and
    a warning is printed.
    """
    lens = np.asarray(lengths, dtype=np.float32)
    assert lat2d.shape[0] == lens.shape[0]

    # swap if needed
    if len_min is not None and len_max is not None and len_min > len_max:
        len_min, len_max = len_max, len_min

    mask = np.ones_like(lens, dtype=bool)
    ...
    print(
        "[Len-Highlight] range = [{}, {}], highlight points = {} / {}".format(
            len_min, len_max, n_highlight, lens.shape[0]
        )
    )
    ...
    if n_highlight > 0:
        ax.scatter(
            ...
            lat2d[mask, 1],
            s=8,
            c="#fb923c",  # bright orange
            alpha=0.9,
            edgecolors="none",
            label="length in [{}, {}]".format(int(len_min), int(len_max)),
        )
    else:
        print("[Len-Highlight] no points in the specified range.")

    ax.set_xlabel("t-SNE dim-1")
    ax.set_ylabel("t-SNE dim-2")
    ax.set_title(
        "t-SNE: length in [{}, {}] highlighted (n = {})".format(
            int(len_min), int(len_max), n_highlight
        )
    )
...
def plot_tsne_ss_argmax(lat2d, helix_frac, sheet_frac, loop_frac, out_png, title):
    """t-SNE colored by argmax of (helix_frac, sheet_frac, loop_frac)."""
...
def plot_tsne_cath_topclass(lat2d, labels, out_png, title):
    """t-SNE colored by top-level CATH class (1,2,3,4,6)."""
    if labels is None:
...
def plot_tsne_fold_topk(lat2d, cath_full, out_png, title, top_k=5):
    """t-SNE colored by top-K most frequent fold prefixes."""
    ...
    # Remove "unknown" if it exists; handle separately
    ...
    num_groups = len(keep_folds) + 1  # +1 for "others"
...
# ---------------------------------------------------------------------
# Clustering on t-SNE (KMeans) and cluster-level analysis
# ---------------------------------------------------------------------

def cluster_tsne_kmeans(lat2d, n_clusters=6, random_state=42):
    """Run KMeans on t-SNE coordinates, return cluster labels [N]."""
    N = lat2d.shape[0]
    if N < n_clusters:
        n_clusters = max(1, N)
    if n_clusters <= 1:
...
def plot_tsne_clusters(lat2d, cluster_labels, out_png, title):
    """t-SNE colored by KMeans cluster id."""
...
def write_cluster_summary(
    ...
    print("[Cluster] total points = {}, clusters = {}, main cluster = {} (n={})".format(
    ...
    with open(summary_path, "w") as f:
        ...
        for cid, size in sorted_clusters:
            ...
            len_sub = np.asarray(lengths, dtype=np.float32)[idx] if lengths is not None else None
            h_sub = np.asarray(helix_frac, dtype=np.float32)[idx] if helix_frac is not None else None
            s_sub = np.asarray(sheet_frac, dtype=np.float32)[idx] if sheet_frac is not None else None
            l_sub = np.asarray(loop_frac, dtype=np.float32)[idx] if loop_frac is not None else None

            f.write("Cluster {}:\n".format(cid))
            f.write("  size = {} ({:.4f} of total)\n".format(size, frac))

            if len_sub is not None:
                f.write("  length: mean={:.2f}, median={:.2f}, min={:.2f}, max={:.2f}\n".format(
                    float(len_sub.mean()),
    ...
    # Island curves: all clusters except main_cluster
    if rel_paths is not None:
        ...
        # Also per-cluster curve list
        for cid, size in sorted_clusters:
            idx = np.where(lab == cid)[0]
            out_txt = os.path.join(analysis_dir, "cluster_{}_curves.txt".format(cid))
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=float,
        default=None,
        help="Optional lower bound of length range for highlight plot",
    )
    p.add_argument(
        "--highlight_len_max",
        type=float,
        default=None,
        help="Optional upper bound of length range for highlight plot",
    )
    return p.parse_args()
...
def main():
    ...
    if lengths is not None:
        ...
        # Optional highlight plot if both bounds are provided
        if args.highlight_len_min is not None and args.highlight_len_max is not None:
            out_name = "tsne_len_highlight_{}_{}.png".format(
                int(args.highlight_len_min),
                int(args.highlight_len_max),
            )
            plot_tsne_highlight_length(
                tsne_2d,
                lengths,
                len_min=float(args.highlight_len_min),
                len_max=float(args.highlight_len_max),
                out_png=os.path.join(analysis_dir, out_name),
            )
        elif (args.highlight_len_min is not None) or (args.highlight_len_max is not None):
            print(
                "[Warn] highlight_len_min / highlight_len_max must both be set; "
                "skip length-highlight plot."
            )
    ...
    for cid in cluster_ids:
        ...
        if lengths is not None:
            plot_cluster_hist(
                ...
                os.path.join(analysis_dir, "hist_length_cluster_{}.png".format(cid)),
        ...
        if helix_frac is not None:
            plot_cluster_hist(
                ...
                os.path.join(analysis_dir, "hist_helix_frac_cluster_{}.png".format(cid)),
        ...
        if sheet_frac is not None:
            plot_cluster_hist(
                ...
                os.path.join(analysis_dir, "hist_sheet_frac_cluster_{}.png".format(cid)),
        ...
        if loop_frac is not None:
            plot_cluster_hist(
                ...
                os.path.join(analysis_dir, "hist_loop_frac_cluster_{}.png".format(cid)),
                title="Loop fraction (cluster {})".format(cid),
                xlabel="loop fraction",
            )

    # 8) Cluster-level summary + island curve list
