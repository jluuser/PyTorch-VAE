...
# coding: utf-8
"""
Decode fixed-N code indices or latent z_q to curves with a trained VQVAE.
...
For backward compatibility:
  - If a manifest record has "indices_path", we treat it as discrete codes.
  - If a record has "latent_path", we treat it as continuous latent z_q.
"""
...
@torch.no_grad()
def decode_one_from_indices(core, indices_np: np.ndarray, target_len: int) -> torch.Tensor:
    """
    Decode from discrete indices (legacy path).
    """
    dev = next(core.parameters()).device
...
@torch.no_grad()
def decode_one_from_latent(core, zq_np: np.ndarray, target_len: int) -> torch.Tensor:
    """
    Decode directly from continuous latent z_q (used by diffusion prior).

    zq_np is expected to be either:
      - [N_tokens, D]
      - or [1, N_tokens, D]
    """
    dev = next(core.parameters()).device
    ...
    if zq_np.ndim == 2:
        # [N, D] -> [1, N, D]
        z_q = torch.from_numpy(zq_np).unsqueeze(0).to(dev)
    elif zq_np.ndim == 3 and zq_np.shape[0] == 1:
        z_q = torch.from_numpy(zq_np).to(dev)
    else:
        raise ValueError(f"zq_np must be [N,D] or [1,N,D], got {zq_np.shape}")
    ...
    # Try the same decode signatures as indices path
    try:
        out = core.decode(z_q, mask=mask, target_len=L_target)
    except TypeError:
        try:
...
def main():
    ...
    n_ok = 0
    for r in records:
        # target length (common to both indices and latent paths)
        tlen = int(r["target_len"]) if "target_len" in r else int(r.get("length", 0))
        if tlen <= 0:
            print(f"[warn] invalid target_len for record id={r.get('id', 'NA')}")
            continue

        latent_path_str = r.get("latent_path", "")
        indices_path_str = r.get("indices_path", "")

        if latent_path_str:
            # New path: decode from latent z_q saved as float
            lat_path = Path(latent_path_str)
            if not lat_path.exists():
                print(f"[warn] missing latent: {lat_path}")
                continue

            zq_np = np.load(str(lat_path), allow_pickle=False)
            if args.check_latent_len > 0 and int(zq_np.shape[0]) != int(args.check_latent_len):
                print(
                    f"[warn] latent_len mismatch {zq_np.shape[0]} != {args.check_latent_len} at {lat_path}"
                )

            recon = decode_one_from_latent(core, zq_np, tlen)  # [1, L, C]
            stem = lat_path.stem

        elif indices_path_str:
            # Legacy path: decode from discrete indices
            idx_path = Path(indices_path_str)
            ...
            recon = decode_one_from_indices(core, idxs, tlen)  # [1, L, C]
            stem = idx_path.stem

        else:
            print(f"[warn] record id={r.get('id', 'NA')} has neither latent_path nor indices_path")
            continue

        save_path = out_dir / (stem + "_recon.npy")
